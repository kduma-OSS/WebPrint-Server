    ...
    <div class="flex w-full items-center justify-between space-x-6 p-6">
        <div class="flex-1 truncate">
            ...
            <p class="mt-1 truncate text-sm text-gray-500">
                {{ $printer->location ?? __('printers.no-location-label') }}
            </p>
        </div>
        ...
        <div class="-mt-px flex divide-x divide-gray-200">
            @can('view', $printer)
                <div class="flex w-0 flex-1">
                    <a href="{{ route('web-print.printers.show', $printer) }}"
                       class="relative -mr-px inline-flex w-0 flex-1 items-center justify-center rounded-bl-lg border border-transparent py-4 text-sm font-medium text-gray-700 hover:text-gray-500">
                        ...
                        <span class="ml-3">{{ __('common.buttons.view-printer') }}</span>
                    </a>
                </div>
            @endcan
            @can('viewAny', [\App\Models\PrintJob::class])
                <div class="-ml-px flex w-0 flex-1">
                    <a href="{{ route('web-print.jobs.index', ['printer' => $printer]) }}"
                       class="relative inline-flex w-0 flex-1 items-center justify-center rounded-br-lg border border-transparent py-4 text-sm font-medium text-gray-700 hover:text-gray-500">
                        ...
                        <span class="ml-3">{{ __('common.buttons.jobs') }} ({{ $printer->jobs_count }})</span>
                    </a>
                </div>
            @endcan
        </div>
